  ...
  <div class="mx-auto max-w-4xl px-4">
    ...
    <div class="bg-gray-900 text-gray-100 p-6 rounded-lg mb-6">
      <div class="flex items-center justify-between mb-4">
        ...
        <button on:click={() => copyToClipboard(`npm install nostr-tools`)} class="flex items-center gap-2 text-xs text-gray-400 hover:text-white">
          <Copy class="h-3 w-3" />
          {copied ? 'Copied!' : 'Copy'}
        </button>
      </div>
    ...
    <div class="bg-gray-900 text-gray-100 p-6 rounded-lg mb-6">
      <div class="flex items-center justify-between mb-4">
        ...
        <button on:click={() => copyToClipboard(`import { generatePrivateKey, getPublicKey, Event, getEventHash, signEvent, relayInit } from 'nostr-tools'

const sk = generatePrivateKey()
const pk = getPublicKey(sk)
console.log('npub:', pk)`)} class="flex items-center gap-2 text-xs text-gray-400 hover:text-white">
          <Copy class="h-3 w-3" />
          {copied ? 'Copied!' : 'Copy'}
        </button>
      </div>
      <pre class="text-sm overflow-x-auto"><code>{`import { generatePrivateKey, getPublicKey, Event, getEventHash, signEvent, relayInit } from 'nostr-tools'
...
console.log('npub:', pk)`}</code></pre>
    ...
    <div class="bg-gray-900 text-gray-100 p-6 rounded-lg mb-6">
      <div class="flex items-center justify-between mb-4">
        ...
        <button on:click={() => copyToClipboard(`import { relayInit, getPublicKey, getEventHash, signEvent } from 'nostr-tools'
...
const pk = getPublicKey(sk)
const ev = { kind: 1, created_at: Math.floor(Date.now()/1000), tags: [], content: 'Hello from TS', pubkey: pk }
ev.id = getEventHash(ev)
ev.sig = signEvent(ev, sk)

const pub = relay.publish(ev)
pub.on('ok', () => console.log('published'))`)} class="flex items-center gap-2 text-xs text-gray-400 hover:text-white">
          <Copy class="h-3 w-3" />
          {copied ? 'Copied!' : 'Copy'}
        </button>
      </div>
      <pre class="text-sm overflow-x-auto"><code>{`import { relayInit, getPublicKey, getEventHash, signEvent } from 'nostr-tools'
